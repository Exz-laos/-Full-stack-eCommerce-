...
//   return (
//     <header className='h-16 shadow-md bg-yellow-100 fixed w-full z-40'>
  ...
  return (
    <header className='h-16 shadow-md bg-yellow-100 fixed w-full z-40'>
      <div className='h-full container mx-auto flex items-center px-4 justify-between'>
        ...
        {/* Mobile Search Input */}
        {mobileSearchVisible && (
          <div className='lg:hidden flex items-center w-full justify-between max-w-sm border rounded-full focus-within:shadow pl-2 absolute top-14 left-0 right-0 mx-4 bg-white z-50'>
            <input 
              type='text' 
          ...
          <div className='relative flex justify-center'>
            ...
            {menuDisplay && (
              <div className='absolute bg-white bottom-0 top-11 h-fit p-2 shadow-lg rounded'>
                <nav>
                  {user?.role === ROLE.ADMIN && (
                    <Link to={'/admin/all-orders'} className='whitespace-nowrap hidden md:block hover:bg-slate-100 p-2'  
                      onClick={() => setMenuDisplay(prev => !prev)}>
                      Admin Panel
                    </Link>
                  )}
